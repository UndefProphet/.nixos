{ inputs, ... }: {
  ...
  flake.modules.nixos.niri =
    ...
    let
      cfg = config.conf.windowManagers.niri;
    in
    {
      imports = [
        {
          options.conf.windowManagers.niri = {
            ...
            settings = {
              ...
              input = {
                keyboard = {
                  xkb = {
                    layout = "us,se";
                    options = "caps:escape,grp:shift_caps_toggle";
                  };
                  numlock = true;
                };
              ...
              spawn-at-startup = [
                ...
                { argv = [ "${lib.getExe pkgs.dunst}" ]; }
                { argv = [ "${lib.getExe pkgs.quickshell}" ]; }
                ...
                { argv = [ "${lib.getExe pkgs.networkmanagerapplet}" ]; }
                # { argv = [ lib.getExe pkgs.pasytray ]; }
                { argv = [ "${pkgs.blueman}/bin/blueman-applet" ]; }
                {
                  argv = [
                    "${lib.getExe pkgs.qpwgraph}"
                    "--minimized"
                  ];
                }
                {
