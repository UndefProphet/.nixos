    ...
    { config, username, ... }:
    {
      ...
      config = {
        ...
        virtualisation.vmVariant = {
          ...
          virtualisation.memorySize = 2048; # Memory in megabytes (e.g., 2GB = 2048)
          virtualisation.cores = 2; # Optional: increase CPU cores as well
          # 1. Enable autologin inside the VM only
          services.getty.autologinUser = username;

          conf.environment.desktop-manager.greetd.enable = lib.mkForce false;
